...
import { createServiceClient, SUPABASE_URL, SERVICE_ROLE_KEY } from '../_shared/supabase-admin.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    // All modes (single conversation, backfill, inactive) require the CRON_SECRET.
    // The previous DB trigger that fired this function with the public anon key has
    // been removed; callers must present `Authorization: Bearer <CRON_SECRET>`.
